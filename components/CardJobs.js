...
function CardJobs({title, place, img, link, color, id}) {
   
  return (
    <a key={id}  href={link} className="w-full h-[480px] group relative rounded-xl border border-black overflow-hidden ">
    ...
    <div className="flex flex-row w-full justify-between items-start absolute p-8 group-hover:opacity-0">
      ...
      <h3 className="text-2xl font-medium text-black relative z-10 ">Desarrollo para {place}</h3>
      </div>
